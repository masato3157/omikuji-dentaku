...
### ラッキーアイテム機能実装
* **「広告」から「エンタメ」へ**:
    * 結果詳細モーダルの中段（旧広告エリア）を「ラッキーアイテム」表示エリアに変更。
    * おみくじ結果に応じてランダムなアイテムを提案し、Amazon検索結果へ誘導する機能を実装。
* **データ構造**:
    * `fortuneResult` に基づくアイテム抽選ロジックを実装。
    * 「凶」の場合のみ、「虚無（禅の本）」などの特定アイテムが表示されやすくなる救済（？）ロジックを追加。
    * アイテム定義は `src/constants/fortunes.js` に集約（アイコン、検索クエリ、商品名）。
* **UIデザイン**:
    * 広告バナーのような違和感を排除し、アプリの世界観に馴染む「白いカード＋オレンジ枠」のデザインを採用。
    * アイコン（Lucide React）とテキストのみで構成し、シンプルさを維持。

### 環境設定
* **アソシエイトIDの外部化**:
    * AmazonアソシエイトIDをソースコードから分離し、環境変数（`.env`）で管理する仕組みを導入。
    * 変数名: `VITE_AMAZON_ASSOCIATE_ID`

### 変更したファイル
* `src/constants/config.js` - 新規作成（機能フラグ）
* `src/utils/amazon.js` - 新規作成（URL生成）
* `src/constants/fortunes.js` - ラッキーアイテムデータ追加
* `src/App.jsx` - ラッキーアイテム表示ロジック、バナー実装、不要タグ削除（バグ修正）
* `.env` - 新規作成
* `.gitignore` - `.env` 除外設定追加
* `package.json` - バージョン 0.9.0
...
### SNSシェア機能実装
* **LINEミニアプリ対応シェア**:
    * 計算結果を X (旧Twitter) および LINE でシェアするボタンを実装。
    * **LINEミニアプリ連携**: `liff.isInClient()` を判定に用い、ミニアプリ内では外部ブラウザ経由でシェア画面を開く適切な挙動を実装。
    * シェアされるURLは Web版ではなく LINEミニアプリのURL (`miniapp.line.me`) とし、新規ユーザー獲得を最適化。
* **コンポーネント分離**:
    * シェアボタンロジックを `src/components/ShareButtons.jsx` に分離し、`App.jsx` を肥大化させずに機能を拡張。

### 画面レイアウト最適化
* **スマホ向け縦幅圧縮**:
    * 結果詳細モーダル（おみくじ結果画面）がスマホ画面からはみ出さないよう、徹底的な余白カットを実施。
    * シェアボタン上部、おみくじ詳細エリア、ヘッダー等のマージン・パディングを限界まで縮小。
    * フッター（利用規約リンク）の境界線を削除し、電卓本体の直下に配置することで省スペース化を実現。

### 環境設定のリファクタリング
* **設定ファイル集約**:
    * ハードコードされていた LINEミニアプリのURL を `src/constants/config.js` (`LINE_MINI_APP_URL`) に移動。
    * 開発用URLから本番用URLへの切り替えを、設定ファイルの1行変更のみで行えるようにアーキテクチャを改善。

### 変更したファイル
* `src/components/ShareButtons.jsx` (新規作成)
* `src/App.jsx`
